...
import { getOngoingAnime } from "@/lib/data";
...
export default async function OngoingPage({ searchParams }: OngoingPageProps) {
  ...
  const { animeList, pagination } = await getOngoingAnime(currentPage);
      ...
      <div className="container px-4 md:px-8 lg:px-12 pt-24 max-w-none">
        <div className="mb-12">
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight uppercase mb-2 border-l-4 border-[#ac4bff] pl-4">
            Ongoing <span className="text-[#ac4bff]">Anime</span>
          </h1>
          ...
          <div className="flex justify-center items-center gap-4 mt-12">
            ...
            >
              {pagination.hasPrevPage ? (
                <Link href={`/ongoing?page=${pagination.prevPage}`}>
            ...
            >
              {pagination.hasNextPage ? (
                <Link href={`/ongoing?page=${pagination.nextPage}`}>
